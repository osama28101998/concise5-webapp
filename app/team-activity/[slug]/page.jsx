...
const DetailPage = () => {
  ...
  const firstName = searchParams.get("fname") || "Unknown";
  // const firstName = router.query?.fname || "Unknown";
  const requestBody = {
    userId: slug,
    type: "user",
  };
  ...
  // console.log("🚀 ~ DetailPage ~ firstName:", firstName)
  ...
  const handleAccordionClick = async (sub_category_id) => {
    if (titlesByCategory[sub_category_id]) return;  
  ...
  const handleClick = (data) => {
    ...
    router.push(`/team-activity/${slug}/${data.id}`);
  };

    useEffect(() => {
    if (firstName) {
      document.title = `Concise-5 | team | ${firstName}`;
    }
  }, [firstName]);
  return (
    <>
      <div className="flex flex-col items-center p-5 space-y-5">
        <div className="bg-blue-700 text-center text-white p-5 rounded-md w-full max-w-2xl">
          {firstName}'s Personal Library Of Tutorials Taken
        </div>
      </div>
